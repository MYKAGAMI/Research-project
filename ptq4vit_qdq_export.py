...
# 导出 Q/DQ ONNX（激活/权重均先用 per-tensor，导出稳），
# 同时把每个 Linear 的“逐通道”权重 scale 存到 .npz，供后处理写回 INT8 per-channel。
...
class QDQLinearExport(nn.Module):
    ...
    真正 per-channel 在导出后由 ONNX 后处理脚本完成。
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        a_q = torch.quantize_per_tensor(x, float(self.a_scale.item()),
                                        int(self.a_zero_point.item()), torch.quint8)
        a_dq = torch.dequantize(a_q)
        W = self.inner.weight
        w_q = torch.quantize_per_tensor(W, float(self.w_scale.item()),
                                        int(self.w_zero_point.item()), torch.qint8)
...
# -------------------- get qparams from PTQ4ViT wrappers -----------------------
...
def _replace_linears_with_qdq_and_dump_scales(model: nn.Module,
    ...
    for name, lin in linears.items():
        ...
        if w_scale_tensor is None or (w_scale_tensor.numel() not in (1, W.shape[0])):
            # fallback: per-channel from |W|_max
...
def export_qdq_onnx_from_model(model: nn.Module,
    ...
    qdq_model = _replace_linears_with_qdq_and_dump_scales(model, wrapped_modules,
                                                          act_ranges, scales_npz).eval().to(device)
    ...
    torch.onnx.export(
        ...
        opset_version=opset,
        do_constant_folding=False,  # 关键：禁止折叠 Q->DQ(Constant)，避免回退到 FP32
